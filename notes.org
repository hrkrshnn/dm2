...
#+author: Hari
#+LATEX_HEADER: \usepackage[left=1cm, right=1cm, bottom=2cm, top=1cm]{geometry}
...
*** Exercise 3
 The following is the proof by Yugin.
    ...
    Brook's theorem.
*** Exercise 3
    [[https://math.stackexchange.com/questions/454353/show-that-triangle-free-planar-graphs-are-four-colorable][Math SO]]
*** Exercise 4
...
*** Problem 3
    ...
    *Bonus* Prove some upper bound for the general problem. Prove the bound of
     seven.
*** Problem 4
    1. For $n =1$ it's trivial. Assume it's true for $\le n- 1$. Write $f$ in
    ...
    2. $f(x_1, x_2) = x_1x_2$ and $S = (0, 1)$ and $d = 2$, $d_1 = d_2 = 1$,
       $d\vert S \vert^{n-1} = 4$. Thus $\vert S \vert^n - \Prod (\vert S
       \vert - d_i) =3$.
    3. $f = x_1^2 + x_2^2 - 5$ and $S = \{\pm 1, \pm 2\}$. Then we have $2 \cdot
...
** DONE Minimum number of edges in a k connected graph?
   CLOSED: [2019-02-15 Fri 16:05]
...
** TODO Proof of Vizing's theorem
** Theorems in the classs
   1. [ ] Proof of lower bound of sorting.
   2. [ ] Proof of Djistra's algorithm
   3. [ ] Tutte's Theorem and it's complicated proof
   4. [X] 3 regular graph without cut edges does not have perfect matching.
   5. [ ] $k$ regular bipartite matching has a perfect matching.
   6. [ ] Konig's theorem. The IP/LP proof is nice.
   7. [ ] The value of $2\alpha'(G)$ is the minimum of $\min\{n - o(G\setminus
      S) + \vert S \vert\}$. This was an exercise too.
   8. [ ] Algorithm to find maximal matching in a Bipartite graph. Later in the
      course a randomized algorithm was also done.
   9. [ ] Augmenting path algorithm.
   10. [ ] Regular bipartite Graph has a perfect matching <2019-02-17 Sun> (*Frobenius theorem*)
   11. [ ] Hungarian algorithm and it's dual. (Maximum weighted matching and
       minimum weighted cover problem.)
   12. [ ] Proposition 8.4. For all $k \ge 2$, there exists a $k$ connected graph
       with $\lceil (n-k)/2 \rceil$ edges.<2019-02-15 Fri>
   13. [X] *Theorem (Chvtal-Erdos)* 8.5. I feel like this might be a question
       for the exam. <2019-02-15 Fri> It's about a condition that makes the
       graph Hamiltonian. If $\kappa \ge \beta$ where $\beta$ is the maximal
       independent set.
       
       The proof used the following facts: take a maximal cycle. It has to have
       at least $\delta + 1$ edges. Now $\delta \le \kappa$. If this cycle
       disconnects $G$, then take $v_1 \cdots v_i$ to be the edges on the cycle
       that has edges to a disconnected component. There has to be at least $k$
       of them. Take vertices next to each $v_i$. Prove that they form an
       independent set. Now add a random vertex form $H$ to form a larger
       independent set.
   14. [X] Menger's theorem. Vertex menger, Edge menger etc. I feel like this
       might show up for the exam.<2019-02-16 Sat>
   15. [ ] *TODO* Study the proof of local implies global. Case 2.
   16. [X] Ford Fulkerson and it's proof. This might come for the exam. But I
       don't know.<2019-02-16 Sat>
   17. [ ] Study the various correspondence between local version of menger's
       theorem. For example local vertex menger if and only if local edge
       menger. <2019-02-17 Sun>
   18. [X] Proof of vertex Menger using network flows. <2019-02-17 Sun>
   19. [X] Integrability theorem.

       The proof was kinda simple? Maybe I didn't write all the details. The
       argument what that $\varepsilon$ is an integer. Now, at each iteration,
       the flow value increases and the 
   20. [X] Partitioning triplets and the deal about Baryani's theorem

       The ending of Baryani's theorem is too tricky.
   21. [ ] Proposition about $X(G) \ge \frac{v(G)}{\alpha(G)}$ I don't even know
       if the statement is true. I don't have a proof in my lecture.
   22. [X] Hajos conjecture. and Proof for $n = 4$. I feel like this can be a
       possible theorem for the exam. (Dirac theorem)

       I haven't looked at the exercise related to the problem though.
   23. [X] Proof of $X \le \Delta + 1$. I think I need to really study the
       Greedy coloring. I made some mistakes about it in the tutorial.
   24. [X] Brook's theorem and proof. There were a couple of exercises that used
       Brook's theorem to prove stuff. The proof was a bit lengthy and even
       required some structure theorem. Probably this one's not coming. <2019-02-18 Mon>

       I probably need to reread the proof of this theorem.
   25. [X] 5 list color theorem. This was already asked in the mock exam. *Reread*
   26. [X] $X_l(G) \ge X(G)$ <2019-02-17 Sun> The proof is obvious
   27. [X] For Bipartite multigraph. $X'(G) = \Delta(G)$. This is a relatively
       simple proof. Might actally come up for exam.
   28. [ ] *Vizing's theorem* Might come in the exam. Proof not too hard. *Reread*
   29. [ ] $X_l(G) = X'(G)$ for Bipartite graphs. We proved it for $K_{n,n}$.
       Proof not too hard. Might end up in the exam. Okay no. I think the proof
       is really lengthy. *Galvin's theorem*
   30. [ ] Study about Kernels.
   31. [X] *Proposal algorithm* and Galey Shapely. He might ask this for the
       exam. Proof uses Kernel perfect stuff. <2019-02-16 Sat>

       What's the deal with kernel perfect here?
   32. [ ] *Linear Programming*.
   33. [ ] Randomized algorithm for Matching. Especially the example.
   34. [ ] Algorithm 
   35. [ ] Write down all definitions clearly.
   36. [ ] The proof of Gallai's theorem for $N = 7$.
   37. [X] The proof of existence of mixed Nash equilibrium. *Recall the
       definitions of $\beta$ and $\alpha$* I think I'm very likely to mess this
       up.
   38. [X] Min-max theorem for zero sum games.
   39. [X] Tutte's theorem. *Reread*
   40. [ ] Reread the proof of if there is an optimum for the LP, then there is
       a basic feasible solution that attains the optimum.
   41. [ ] Proof of duality.
   42. [ ] Proof of Hall's condition.
** Questions
   1. [ ] What is the difference between an independent set and vertex cover.
** TODO Why are 3 critical graphs an odd cycle?
** TODO Definitions
   1. [ ] Mixed nash equilibrium, mixed strategy, best response etc.
   2. [ ] List coloring
   3. [ ] Chromatic number, list chromatic number.
   4. [ ] Connectivity. Vertex-connectivity and edge connectivity.
** TODO Algorithms to study
   1. [ ] Djistra
   2. [ ] Kruskal
   3. [ ] 
